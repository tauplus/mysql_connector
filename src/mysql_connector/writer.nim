...
func write_zero*(writer: var Writer, len: Natural) =
  writer.add( new_packet(len) )
  return

func write_int_1*(writer: var Writer, val: uint8) =
  writer.add(byte(val))
  return

func write_int_2*(writer: var Writer, val: uint16) =
  writer.add(byte(val))
  writer.add(byte(val shr 8))
  return

func write_int_3*(writer: var Writer, val: uint32) =
  writer.add(byte(val))
  writer.add(byte(val shr 8))
  writer.add(byte(val shr 16))
  return

func write_int_4*(writer: var Writer, val: uint32) =
  writer.add(byte(val))
  writer.add(byte(val shr 8))
  writer.add(byte(val shr 16))
...
func write_int_8*(writer: var Writer, val: uint64) =
  writer.add(byte(val))
  writer.add(byte(val shr 8))
  writer.add(byte(val shr 16))
  writer.add(byte(val shr 24))
...
func write_length_encoded_integer*(writer: var Writer, val: uint64) =
  if val < 251:
...
func write_fixed_length_string*(writer: var Writer, str: string, len: Natural) =
...
func write_null_terminated_string*(writer: var Writer, str: string) =
  let writer_original_len = writer.len()
...
func write_length_encoded_string*(writer: var Writer, str: string) =
